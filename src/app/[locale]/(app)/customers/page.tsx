...
export default async function CustomersPage({
  ...
  const [{ data: customers }, { data: deletions }, logs, subjectsRes, enrollRes] = await Promise.all([
    ...
    isTuition
      ? supabase
          ...
          .eq("organization_id", ctx.organization.id)
      : Promise.resolve({
          data: [] as Array<{ customer_id: string; subject_id: string }>,
        }),
  ]);
  ...
  return (
    <div className="stack" style={{ gap: "1.25rem" }}>
      <PageHeader title={title} subtitle={isTuition ? t("tuitionSubtitle") : undefined} />
        ...
        <ActionForm action={upsertCustomerAction} className="stack">
          ...
          {isClinic ? (
            <div className="field">
              ...
              <input name="allergies" className="input" placeholder={t("allergiesPlaceholder")} />
            </div>
          ) : null}
          <div className="field">
            <label>{t("notes")}</label>
        ...
        <div className="surface" style={{ padding: "1.25rem" }}>
          ...
          <div className="table-wrap">
            <table className="data">
              ...
              <tbody>
                {(customers || []).map((c) => {
                  const enrolled = subjectsByCustomer.get(c.id) || [];
                  return (
                    <tr key={c.id}>
                      <td>{c.name}</td>
                      <td>{enrolled.length ? enrolled.join(", ") : "—"}</td>
                    </tr>
                  );
                })}
                {!customers?.length ? (
